...
def down_sample(dense_pcd_path, dense_label_path, sparse_pcd_path,
    ...
    if dense_labels is not None:
        ...
        if len(dense_pcd.points) != len(non_zero_indexes):
            print("somethnig wrong")
            print(
                f"dense_pcd.points:{len(dense_pcd.points)} vs non_zero_indexes:{len(non_zero_indexes)}"
            )
            return
    ...
    sparse_pcd, cubics_ids, some_list = open3d.geometry.PointCloud.voxel_down_sample_and_trace(
        dense_pcd, voxel_size, min_bound, max_bound, approximate_class=False)
...
if __name__ == "__main__":
    ...
    for file_prefix in all_file_prefixes:
        ...
        sparse_label_path = os.path.join(downsampled_dir,
                                         file_prefix + ".labels")
